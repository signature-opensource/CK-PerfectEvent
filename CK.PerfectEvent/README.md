...
async Task ThingTalkedAsync( IActivityMonitor monitor, IThing thing, string e, CancellationToken cancel )
{
...
async Task ThingTalkedAsync( IParallelLogger logger, IThing thing, string e, CancellationToken cancel )
{
...
A Perfect Event is implemented thanks to a [`PerfectEventSender<TEvent>`](PerfectEventSender{TEvent}.cs)
or [`PerfectEventSender<TSender,TEvent>`](SenderAndArg/PerfectEventSender{TSender,TEvent}.cs)
...
public async Task<bool> SafeRaiseAsync( IActivityMonitor monitor, TSender sender, TEvent e, CancellationToken cancel = default, [CallerFilePath] string? fileName = null, [CallerLineNumber] int lineNumber = 0 )
```
...
`CreateBridge` returns a [`IBridge : IDisposable`](IBridge.cs): if needed a bridge can
...
intReceived.ShouldHaveSingleItem().ShouldBe( 3712, "string -> int done." );
```
...
for `PerfectEventSender<TSender,TEvent>`):
```csharp
/// <summary>
